...
$vendas_query = "SELECT SUM(valor) AS total_vendas FROM vendas WHERE MONTH(data) = MONTH(CURRENT_DATE)"; // Exemplo para vendas do mês
$vendas_result = $conn->query($vendas_query);
$vendas_data = $vendas_result->fetch_assoc();
$vendas_mes = $vendas_data['total_vendas'] ?? 0; // Caso não haja vendas, será 0

$gastos_query = "SELECT SUM(valor) AS total_gastos FROM gastos WHERE MONTH(data) = MONTH(CURRENT_DATE)"; // Exemplo para gastos do mês
